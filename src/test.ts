...
describe('getChannelValueFromUnweightedChannelLuminance', () => {
  it.each([
    ...
    'getChannelValueFromUnweightedChannelLuminance '
    + 'reverses getUnweightedChannelLuminance for %d',
    (channelValue) => {
...
describe('getLuminance (exact)', () => {
  ...
  ]).it('Calculates luminance for %s', (hex, expectedLuminance) => {
    const luminance = getLuminance(hex);

    expect(
      luminance.toFixed(4),
      `Expected ${hex} to have luminance ${expectedLuminance}. `
      + `Actual ${luminance}`,
...
describe('getContrastRatio (approx)', () => {
  it.each([
    ...
    'hex colors #%s and #%s yield expected ratio %d',
    (hexA, hexB, expected) => {
      ...
      expect(
        ...
        `Expected contrast between ${hexA} and ${hexB} to be ${expected}. `
        + `Actual ${ratio}`,
      ).toBeLessThan(0.02);
...
const sampleColorsAndLuminances = [
  [
    '#0000ff',
    0.8,
  ],
...
const sampleColorsLuminancesAndExpectations = [
  [
    '#ffffff',
    0,
    '#000000',
...
describe('guessColorByLuminanceWithHsl', () => {
  each(sampleColorsAndLuminances).it(
    'Can make a color with the same hue and saturation as %s but luminance %d',
    (initHex, targetLuminance) => {
      ...
      const [
        ...
        initSaturation1,
        initLightness,
      ] = colorConvert.hex.hsl(initHex);

      const [
        newHue,
        newSaturation,
        newLightness,
      ] = colorConvert.hex.hsl(newHex);
      ...
      expect(
        ...
        `Expected luminance to be approximately ${targetLuminance}.` +
          ` Actual ${actualLuminance}`,
      ).toBeLessThan(0.03);
    });

  it.each(sampleColorsLuminancesAndExpectations)(
    'Gets expected color',
    (hex, luminance, expected) => {
...
const sampleContrastAndColorAndBackground = [
  [
    21,
    '#ffffff',
    '#ffffff',
...
describe('guessColorByContrastWithHsl', () => {
  each(sampleContrastAndColorAndBackground).it(
    'Generating color with contrast %d from color %s on a %s background',
    (expectedContrastRatio, hex) => {
      ...
      expect(
        ...
        `Expected generated color ${color} to have contrast of`+
        ` ${expectedContrastRatio}. Actual ${actualContrastRatio}`,
      ).toBeLessThan(0.3);

    // Fails for 21, #ffffff, #ffffff
    // expect(
    //   Number(actualContrastRatio),
    //   `Expected generated color ${color} to have contrast of at least` +
    //   ` ${expectedContrastRatio}. Actual ${actualContrastRatio}`
    // ).not.toBeLessThan(expectedContrastRatio);
    });

  it(
    'Can make lighter or darker colors depending on the background color',
    () => {
      ...
      expect(
        colorWithLightBase,
        'Expected light and dark bases to produce different results for the' +
        ' same target contrast and hex color.',
      ).not.toEqual(colorWithDarkBase);

      expect(
        contrastOnLightBase - targetContrastRatio,
        `Expected ${colorWithLightBase} to have contrast of` +
        ` ${targetContrastRatio} against ${lightBase}.` +
        ` Actual ${contrastOnLightBase}`,
      ).toBeLessThan(0.3);

      expect(
        Number(contrastOnLightBase),
        `Expected ${colorWithLightBase} to have contrast of at least` +
        ` ${targetContrastRatio} against ${lightBase}.` +
        ` Actual ${contrastOnLightBase}`,
      ...
      expect(
        ...
        `Expected ${colorWithDarkBase} to have contrast of` +
        ` ${targetContrastRatio} against ${darkBase}.` +
        ` Actual ${contrastOnDarkBase}`,
      ).toBeLessThan(0.3);

      expect(
        Number(contrastOnDarkBase),
        `Expected ${colorWithDarkBase} to have contrast of at least` +
        ` ${targetContrastRatio} against ${darkBase}.`+
        ` Actual ${contrastOnDarkBase}`,
      ).not.toBeLessThan(targetContrastRatio);
    });
